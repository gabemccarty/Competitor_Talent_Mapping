...
(function (global) {
  ...
  function buildDepartmentsForEmployees(employees) {
    ...
    for (let i = 0; i < employees.length; i++) {
      ...
      depts[dept].hierarchy[tierStr].employees.push({ name: emp.name, title: emp.title, location: emp.location, linkedin: emp.linkedin });
      depts[dept].total++;
    }
  ...
  function processCSV(csvText, companyFilter) {
    ...
    return {
      companyName: companyFilter || 'Company',
  ...
  global.TalentMapParser = { processCSV: processCSV, tierLevelNames: tierLevelNames, departmentColors: departmentColors };
