...
function Globe2D(container, options) {
  ...
  canvas.addEventListener('mouseleave', function () { self.onUp(); });
...
Globe2D.prototype.getPinAt = function (clientX, clientY) {
  ...
  for (var i = 0; i < (this.pinPositions || []).length; i++) {
    ...
    var dx = x - p.x;
    var dy = y - p.y;
    if (dx * dx + dy * dy <= this.pinHitRadius * this.pinHitRadius) {
      var loc = p.location;
      return {
        label: loc.label != null ? loc.label : unknownLocation.label,
        count: loc.count != null ? loc.count : 0,
        departments: loc.departments || {},
        lat: loc.lat != null ? Number(loc.lat) : unknownLocation.lat,
        lng: loc.lng != null ? Number(loc.lng) : unknownLocation.lng,
      };
    }
  }
  return null;
